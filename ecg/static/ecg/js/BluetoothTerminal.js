...
class BluetoothTerminal {
  ...
  send(data) {
    ...
    if (!data) {
      return Promise.reject(new Error('Escribe un número'));
    }
    ...
    if (!this._characteristic) {
      return Promise.reject(new Error('Dispositivo no conectado'));
    }
    ...
    for (let i = 1; i < chunks.length; i++) {
      ...
      promise = promise.then(() => new Promise((resolve, reject) => {
        ...
        if (!this._characteristic) {
          reject(new Error('Se ha desconectado el dispositivo'));
        }
  ...
  _disconnectFromDevice(device) {
    ...
    this._log('Desconectado de "' + device.name + '" dispositivo bluetooth ...');
    ...
    this._log('"' + device.name + '" dispositivo bluetooth desconectado');
  }
  ...
  _requestBluetoothDevice() {
    this._log('Solicitando dispositivo bluetooth ...');

    return navigator.bluetooth.requestDevice({
      filters: [{services: [this._serviceUuid]}],
    }).
        then((device) => {
          this._log('"' + device.name + '" Dispositivo bluetooth seleccionado');
  ...
  _connectDeviceAndCacheCharacteristic(device) {
    ...
    this._log('Conectandose al GATT server...');

    return device.gatt.connect().
        then((server) => {
          this._log('GATT server conectado', 'Obteniendo servicio...');

          return server.getPrimaryService(this._serviceUuid);
        }).
        then((service) => {
          this._log('Servicio encontrado', 'Obteniendo característica...');

          return service.getCharacteristic(this._characteristicUuid);
        }).
        then((characteristic) => {
          this._log('Característica encontrada');
  ...
  _startNotifications(characteristic) {
    this._log('Iniciando notificaciones...');

    return characteristic.startNotifications().
        then(() => {
          this._log('Notificaciones iniciadas');
  ...
  _stopNotifications(characteristic) {
    this._log('Deteniendo notificaciones...');

    return characteristic.stopNotifications().
        then(() => {
          this._log('Notificaciones detenidas');
  ...
  _handleDisconnection(event) {
    ...
    this._log('"' + device.name +
        '" dispositivo bluetooth desconectado, tratando reconectar...');
